...
public class IOUtil {
		...
		if (lastThr != null) {
			...
			// should not happen
				...
				throw (IOException) new IOException(
						"Unexpected exception during close").initCause(lastThr);
			}
		}
	 ...
	 * where file is wiped on close and thus the disk resource is released ('closed').
	 * 
	 * @author dmitriy
	 *
	 */
    public  static class DeleteFileOnClose implements Closeable {
        ...
        public void close() throws IOException {
            if ( m_file.isFile()) m_file.delete();
        } 
